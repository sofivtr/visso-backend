...
public class CarritoController {
    ...
    @Operation(summary = "Cerrar carrito y realizar compra", security = @SecurityRequirement(name = "bearerAuth"))
    public ResponseEntity<MensajeResponse> cerrarCarrito(@PathVariable @Parameter(description = "ID del usuario") Long usuarioId) {
        carritoService.cerrarCarrito(usuarioId);
        return ResponseEntity.ok(new MensajeResponse("Compra realizada con éxito."));
    }

    @GetMapping("/cerrados")
    @Operation(summary = "Listar carritos cerrados", security = @SecurityRequirement(name = "bearerAuth"))
    public ResponseEntity<List<Carrito>> obtenerCarritosCerrados() {
        return ResponseEntity.ok(carritoService.listarVentas());
    }

    @GetMapping("/ventas")
    @Operation(summary = "Listar ventas", security = @SecurityRequirement(name = "bearerAuth"))
